...
async function handlePayment(e) {
    e.preventDefault();
    const user = getUser();
    if (!user || !user.userId) { showToastGlobal('Please log in first', 'error'); window.location.href = '/index.html'; return; }
...
async function loadOrders() {
    ...
    if (!wrap)    return;
    if (!user)    { wrap.innerHTML = '<p class="text-center">Please <a href="/index.html">log in</a> to view orders.</p>'; return; }
    ...
    try {
        ...
        if (!orders.length) { wrap.innerHTML = '<div class="empty-cart"><i class="fa-solid fa-box-open fa-3x"></i><h3>No orders yet</h3><p>Your orders will appear here.</p><a href="/home.html" class="btn-primary">Shop Now</a></div>'; return; }
